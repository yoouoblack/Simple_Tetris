...
public class PlayManager {
	...
	public void update() {
		...
		// Check if the currentMine is active
		if(currentMino.active == false) {
			...
			// when amino becomes inactive, check if line(s) can be deleted
			checkDelete();
		}
		else {
			currentMino.update();
		...
		while(x < right_x && y < bottom_y) {
			...
			if(x == right_x) {
				...
				if(blockCount == 12) {
					...
					for(int i = 0; i < staticBlocks.size(); i++) {
						// if a block is above the current y, move if down by the block size
						if(staticBlocks.get(i).y < y) {
	...
	public void draw(Graphics2D g2) {
		...
		//Draw Next Mino Frame(다음에 나올 블럭을 표시하는 영역)
		int x = right_x + 100;
		int y = bottom_y - 200;
